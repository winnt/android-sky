	...
	public static RemoteViews buildUpdate(Context context, Uri appWidgetUri) {
		...
		try {
			...
			if (cursor != null && cursor.moveToFirst()) {
				...
				// "/"
				// + context.getPackageName() + "/skins/" + skinName +
				// "/widget_bg.png").getBitmap();
				//
				// if (bitmap != null)
				// {
				//
				// views..findViewById(R.id.widget_background);
				// ImageView mBackground = (ImageView)
				// context.getResources().getLayout(R.id.widget_background);
				//
				// int imageIdentifier =
				// context.getResources().getIdentifier(symbolIdentifier, null,
				// null);
				// Drawable d =
				// Drawable.createFromPath(Environment.getExternalStorageDirectory()
				// + "/"
				// + context.getPackageName() + "/skins/" + skinName +
				// "/widget_bg.png");
